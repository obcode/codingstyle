 ...
 */
public class LineRange11 extends LineRange {
    private final int from;
    private final int to;

    public LineRange11(final int line) {
        this(line, line);
    }

    public LineRange11(final int from, final int to) {
        super(1, 3);

        Ensure.that(to > 0).isTrue("From must be a positive number: %d", from);
