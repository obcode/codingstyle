 ...
 */
public class LineRange3 extends LineRange {
    private final int from;
    private final int to;

    public LineRange3(final int line) {
        this(line, line);
    }

    public LineRange3(final int from, final int to) {
        super(1, 3);

        Ensure.that(from > 0).isTrue("From must be a positive number: %d", from);
        Ensure.that(from > 0).isTrue("To must be a positive number: %d", from);
